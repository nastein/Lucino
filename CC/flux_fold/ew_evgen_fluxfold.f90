program ew_eventgen
   ...
   real*8 :: progress,Q2min
   integer*4 :: nw,nev,i,nZ,xA,i_fg,np0,np,ne,j,ilept,gen_events,num_events,nenu
   real*8 :: wmax,enu,thetalept,xpf,hw,sig,sig_err,xmlept,start,finish,total_sig
   integer*8 :: irn=623
   ...
   fname='test.out'
   !fname='C12_CC_enu_'//trim(en_char)//'.out'
   fname=trim(fname)
   ...
   call mc_init(i_fg,nev,xpf,xmlept,xA,nZ,np,np0,ne,Q2min,flux_fname,nenu)
   num_events = 0
   ...
   do while(num_events.le.gen_events)
      ...
      call mc_eval(sig,sig_err,saved_events,irn)
   ...
   !print '("Generated", I7 ," events.")',num_events
