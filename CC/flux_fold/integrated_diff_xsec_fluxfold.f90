module mc_module
   use event_module
   implicit none 
   integer*4, private, save :: nev,xA,nZ,i_fg,np0,np,ne,nenu
   ...
   real*8, private, save:: q2min,flux_norm,enu_max
   ...
   real*8, allocatable, save :: enu_v(:), flux_v(:)
contains

subroutine mc_init(i_fg_in,nev_in,xpf_in,mlept_in,xA_in,nZ_in,np_in,np0_in,ne_in,q2min_in,flux_fname,nenu_in)
   use mathtool
   use event_module
   implicit none
   integer*4 :: nev_in,nZ_in,xA_in,i_fg_in,np_in,i,j,ne_in,np0_in,ne0,ien,nenu_in
   integer*8 :: irn_in
   real*8 :: xpf_in,mlept_in,hp,he,thetalept_in,dummy,q2min_in
   real*8 :: enu_1,enu_2,henu
   character*40 :: flux_fname
   ...
   write(6,*)'Enu max = ', enu_max

end subroutine

subroutine mc_eval(r_avg, r_err, my_events, irn)
   use event_module
   use mathtool
   use dirac_matrices
   implicit none
   ...
   wmax=enu_max-mlept
   q2max_c=2.0d0*enu_max**2-mlept**2+2.0d0*enu_max*sqrt(enu_max**2-mlept**2)
   q2max=q2max_c
   ...
   do iv=1,nev 
      ...
      if(iv.ge.neq.and.mod(iv,nvoid).eq.0) then
         ...
         !Range of neutrino energies we are choosing from
         enu_i = enu_max - w_o - mlept

         !Random neutrino energy 
         Enu = enu_max - enu_i + (enu_i)*ran()

         !write(6,*)'enu = ', Enu
         !write(6,*)'w = ', w_o
         !write(6,*)'Q2 = ', q2_o

         !flux at that neutrino energy
         call interpolint(enu_v,flux_v,nenu,Enu,flux,3)
         ...
         f_o=f_o*(2.0d0*pi) *1.e12*dble(nZ)/g_o * (flux/flux_norm) * enu_i
