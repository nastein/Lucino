module mc_module
   ...
   real*8, private,parameter :: G_F =1.1664e-11*hbarc**2, cb=0.9740d0
...
subroutine mc_eval(Enu, thetalept_in, xsec, xsec_err, my_events)
   ...
   call mc_random_startpoint(g_o,j1_o,j2_o,nk,nk_norm)
   ...
   do i=1,nev
      do j=1,nwlk
         ...
         call mc_step(j1_o(j),j2_o(j),g_o(j),i_acc,nk,nk_norm)
...
subroutine mc_random_startpoint(g,j1,j2,nk,nk_norm)
   integer*4 :: i
...
subroutine mc_step(j1_o,j2_o,g_o,i_acc,nk,nk_norm)
   integer*4 :: j1_n,j2_n
   real*8,intent(in) :: nk(np,np),nk_norm
...
subroutine int_eval(kprobe_4,klept_4,phipp1,ctpp1,p2,ctp2,phip2,p1,ctp1, &
      &  phip1,ip1,ip2,w,q_4,r_now,np1,nuc1P4,nuc2P4,nuc1PP4,nuc2PP4,cosr,phir)
   use dirac_matrices         
   use mathtool
   implicit none
   ...
   real*8 :: at,bt,vt,par1,par2,pp1,den,jac,arg,qval,cosr,phir
