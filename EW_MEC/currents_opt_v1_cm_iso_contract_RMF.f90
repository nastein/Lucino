module dirac_matrices
    ...
    real*8, private, parameter :: mstar = 0.8d0, Ev = 141.0d0
    ...
    real*8, private, save ::  p1(4),p2(4),pp1(4),pp2(4),q(4),k1(4),k2(4),l(4),lp(4)
...
subroutine current_init(lepi_in,lepf_in,p1_in,p2_in,pp1_in,pp2_in,q_in,k1_in,k2_in,i_fl_in,total_isospin_in)
    implicit none
    integer*4 :: i,i_fl_in,total_isospin_in
    real*8 ::  p1_in(4),p2_in(4),pp1_in(4),pp2_in(4),q_in(4),k1_in(4),k2_in(4)
    real*8 :: lepi_in(4),lepf_in(4)
    i_fl=i_fl_in
    pair_isospin=total_isospin_in
    l=lepi_in
    lp=lepf_in
    p1=p1_in
    p2=p2_in
...
subroutine det_JaJb_JcJd(cv3,ca5,np_del,pdel,pot_del)
    ...
    !! Nov 11 (Noah) Add vector energy to delta energies
    pa(:)=p1(:)+q(:) + (/Ev,0.0d0,0.0d0,0.0d0/)
    pb(:)=pp1(:)-q(:) + (/Ev,0.0d0,0.0d0,0.0d0/)
    pc(:)=p2(:)+q(:) + (/Ev,0.0d0,0.0d0,0.0d0/)
    pd(:)=pp2(:)-q(:) + (/Ev,0.0d0,0.0d0,0.0d0/)
    pa2=sum(pa(2:4)**2)
    pb2=sum(pb(2:4)**2)
    pc2=sum(pc(2:4)**2)
    pd2=sum(pd(2:4)**2)
...
!   &     *(pd(1)**2-sum(pd(2:4)**2)-xmd**2)/((pd(1)**2-sum(pd(2:4)**2)-xmd**2)**2+xmd**2*gd**2)
    !!!! Nov 11 (Noah) I mulitplied xmn by mstar because I divide out by that later
         J_a_2(:,:,i,j)=cv3*matmul(g_munu(i,j)*q_sl(:,:)-q(i)*gamma_mu(:,:,j),gamma_mu(:,:,5))+ca5*xmn*mstar*g_munu(i,j)*id4(:,:)
         J_b_1(:,:,i,j)=cv3*matmul(gamma_mu(:,:,5),g_munu(j,i)*q_sl(:,:)-q(j)*gamma_mu(:,:,i))+ca5*xmn*mstar*g_munu(j,i)*id4(:,:)
         J_c_2(:,:,i,j)=cv3*matmul(g_munu(i,j)*q_sl(:,:)-q(i)*gamma_mu(:,:,j),gamma_mu(:,:,5))+ca5*xmn*mstar*g_munu(i,j)*id4(:,:)
         J_d_1(:,:,i,j)=cv3*matmul(gamma_mu(:,:,5),g_munu(j,i)*q_sl(:,:)-q(j)*gamma_mu(:,:,i))+ca5*xmn*mstar*g_munu(j,i)*id4(:,:)
...
! costruisco Jmua, Jmub
   do mu=1,4
      ...
      enddo
   enddo
   !!!! Nov 11 (Noah) I divide xmn by mstar because I multiplied by it earlier
   if(i_fl.eq.1) then
      J_a_mu=J_a*fpik2*fpindk2*sqrt(fpinn2)*fstar/xmpi**2/xmn/mstar
      J_b_mu=J_b*fpik2*fpindk2*sqrt(fpinn2)*fstar/xmpi**2/xmn/mstar
      J_c_mu=J_c*fpik1*fpindk1*sqrt(fpinn2)*fstar/xmpi**2/xmn/mstar
      J_d_mu=J_d*fpik1*fpindk1*sqrt(fpinn2)*fstar/xmpi**2/xmn/mstar
   elseif(i_fl.eq.2)then
      Je_a_mu=J_a*fpik2*fpindk2*sqrt(fpinn2)*fstar/xmpi**2/xmn/mstar
      Je_b_mu=J_b*fpik2*fpindk2*sqrt(fpinn2)*fstar/xmpi**2/xmn/mstar
      Je_c_mu=J_c*fpik1*fpindk1*sqrt(fpinn2)*fstar/xmpi**2/xmn/mstar
      Je_d_mu=J_d*fpik1*fpindk1*sqrt(fpinn2)*fstar/xmpi**2/xmn/mstar
   endif

end subroutine


subroutine det_JaJc_dir(r_av)
   implicit none
   integer*4 :: i1,f1,i,j
   ...
   complex*16 :: ciso1,ciso2,ciso3,ciso4,ciso5
   ...
   jbjb(:,:)=tr_jbjb(:,:)*tr_2*ciso2*2.0d0
   jajb(:,:)=tr_jajb(:,:)*tr_2*ciso3*2.0d0
   do i=1,4
      do j=1,4
      jajc(i,j)=tr_ja(i)*tr_jc(j)*ciso4*2.0d0
      jajd(i,j)=tr_ja(i)*tr_jd(j)*ciso3*2.0d0
      jbjc(i,j)=tr_jb(i)*tr_jc(j)*ciso3*2.0d0
      jbjd(i,j)=tr_jb(i)*tr_jd(j)*ciso5*2.0d0
      enddo
   enddo  
...
!!!! what we want to compute is
   ...
   jajc(:,:)=tr_jajc(:,:)*ciso4*2.0d0
   jajd(:,:)=tr_jajd(:,:)*ciso3*2.0d0
   jbjc(:,:)=tr_jbjc(:,:)*ciso3*2.0d0
   jbjd(:,:)=tr_jbjd(:,:)*ciso5*2.0d0
 ...
 subroutine det_JpiJaJb(r_av)
   ...
   subroutine det_JpiJaJb_exc(r_av)
   implicit none
   integer*4 :: i1,f1,i2,f2,i,j
   real*8 :: r_cc,r_cl,r_ll,r_t,r_tp
   complex*16 :: J_12a(2,2,4),J_12b(2,2,4)
...
subroutine det_JpiJpi(r_av)
   ...
   do mu=1,4
      ...
      enddo
   enddo 
   jsjs(:,:)=ciso1*(jsjs(:,:)+tr_ss11(:,:)*tr_2+tr_ss22(:,:)*tr_1)*2.0d0
   jpjp(:,:)=ciso1*(jpjp(:,:)+tr_pp11(:,:)*tr_2+tr_pp22(:,:)*tr_1)*2.0d0
   jpjs(:,:)=ciso1*(jpjs(:,:)+tr_ps11(:,:)*tr_2+tr_ps22(:,:)*tr_1)*2.0d0
   jfjs(:,:)=ciso1*(tr_fs1(:,:)*tr_2*2.0d0)*2.0d0
   jfjp(:,:)=ciso1*(tr_fp1(:,:)*tr_2*2.0d0)*2.0d0
...
subroutine delta_se(pd2,width,pot)
   implicit none
   real*8 :: pd2,width,kpi,ekpi,eknuc,r2a,rfa,pot,xmr
   width=0.0d0
   !pot=0.0d0!-40.0d0
