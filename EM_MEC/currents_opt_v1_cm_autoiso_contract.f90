module dirac_matrices
    ...
    complex*16, private, save :: t1(2),t2(2),t1p(2),t2p(2),iso(2,2)
...
subroutine current_init(lepi_in,lepf_in,p1_in,p2_in,pp1_in,pp2_in,q_in,w_in,k1_in,k2_in,i_fl_in,iso_in,t1_in,t2_in)
    implicit none
...
subroutine det_JaJb_JcJd(e_gs,e_bg,cv3,ca5,np_del,pdel,pot_del)
    use mathtool
    implicit none
...
subroutine det_JaJc_dir(r_av)
   ...
   !if(pair_isospin.eq.1.or.pair_isospin.eq.0) then
       do i=1,2
          do j=1,2
            do k=1,2
                do l=1,2
                       ciso1=ciso1 + IDeltaADag(iso(i,:),iso(j,:),iso(k,:),iso(l,:))&
                        &   *IDeltaA(iso(k,:),iso(l,:),iso(i,:),iso(j,:))
                       ciso2=ciso2 + IDeltaADag(iso(i,:),iso(j,:),iso(k,:),iso(l,:))&
                        &   *IDeltaC(iso(k,:),iso(l,:),iso(i,:),iso(j,:))
                       ciso3=ciso3 + IDeltaADag(iso(i,:),iso(j,:),iso(k,:),iso(l,:))&
                       &    *IDeltaB(iso(k,:),iso(l,:),iso(i,:),iso(j,:))
                   enddo
               enddo
           enddo
       enddo
...
subroutine det_JaJc_exc(r_exc)
   implicit none
   integer*4 :: i1,f1,i2,f2,i,j,fe,k,l
...
!!!! what we want to compute is
   ...
   !    do i=1,2
   !       do j=1,2
   !            ciso1=ciso1 + IDeltaADag(iso(i,:),iso(j,:),t1,t2)*IDeltaA(t1,t2,iso(j,:),iso(i,:))
   !            ciso2=ciso2 + IDeltaBDag(iso(i,:),iso(j,:),t1,t2)*IDeltaB(t1,t2,iso(j,:),iso(i,:))
   !            ciso3=ciso3 + IDeltaADag(iso(i,:),iso(j,:),t1,t2)*IDeltaB(t1,t2,iso(j,:),iso(i,:))
   !        enddo
   !    enddo
   !if(pair_isospin.eq.1) then
   !     ciso1 = (4.0d0/9.0d0)
   ...
   do i=1,2
          do j=1,2
            do k=1,2
                do l=1,2
                       ciso1=ciso1 + IDeltaADag(iso(i,:),iso(j,:),iso(k,:),iso(l,:))&
                        &   *IDeltaA(iso(k,:),iso(l,:),iso(j,:),iso(i,:))
                       ciso2=ciso2 + IDeltaBDag(iso(i,:),iso(j,:),iso(k,:),iso(l,:))&
                        &   *IDeltaB(iso(k,:),iso(l,:),iso(j,:),iso(i,:))
                       ciso3=ciso3 + IDeltaADag(iso(i,:),iso(j,:),iso(k,:),iso(l,:))&
                       &    *IDeltaB(iso(k,:),iso(l,:),iso(j,:),iso(i,:))
                   enddo
               enddo
   ...
   !print*,'ciso1 = ', ciso1, ', ciso2 = ', ciso2, ', ciso3 = ', ciso3
 ...
 subroutine det_JpiJaJb(r_av)
   ...
   if(pair_isospin.eq.1.or.pair_isospin.eq.0) then
       do i=1,2
          do j=1,2
               ciso1=-1.0d0*(ciso1 - Ivz(iso(i,:),iso(j,:),t1,t2)*IDeltaA(t1,t2,iso(i,:),iso(j,:)))
               ciso2=-1.0d0*(ciso2 - Ivz(iso(i,:),iso(j,:),t1,t2)*IDeltaB(t1,t2,iso(i,:),iso(j,:)))
           enddo
       enddo
   !if(pair_isospin.eq.1) then
   !     ciso1 = 0.0d0
   ...
   if(pair_isospin.eq.1.or.pair_isospin.eq.0) then
       do i=1,2
          do j=1,2
               ciso=-1.0d0*(ciso - Ivz(iso(i,:),iso(j,:),t1,t2)*IDeltaA(t1,t2,iso(j,:),iso(i,:)))
           enddo
       enddo
   !if(pair_isospin.eq.1) then
   !     ciso = 0.0d0
...
subroutine det_JpiJpi(r_av)
   implicit none
   integer*4 :: i1,f1,i2,f2,mu,nu,i,j
   real*8 :: r_cc,r_cl,r_ll,r_t,r_tp   
   ...
   if(pair_isospin.eq.1.or.pair_isospin.eq.0) then
       do i=1,2
          do j=1,2
               ciso=(ciso - Ivz(iso(i,:),iso(j,:),t1,t2)*Ivz(t1,t2,iso(i,:),iso(j,:)))
           enddo
       enddo
   !if(pair_isospin.eq.1) then
   !     ciso = 0.0d0
   ...
   r_av=jsjs!jfjf+jsjs+2.0d0*jfjs+jpjp+2.0d0*jfjp+2.0d0*jpjs
