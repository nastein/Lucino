module mc_module
   ...
   real*8, private, parameter :: pi=acos(-1.0d0),hbarc=197.327053d0,ppmax=3.0d0*1.e3
...
subroutine f_eval(w,i1,i2,i1p,i2p,pj1,pj2,np1,enu_v,f,my_event_in)
   ...
   !Sample initial state kinematics of nucleon 1 (4pi)
   !and phi of nucleon 2
   ...
   call int_eval(probeP4,outlepP4,pj2,phip2, &
      &  pj1,ctp1,phip1,w,q,r_now,np1,nuc1P4,nuc2P4,nuc1PP4,nuc2PP4, &
      &  i1,i2,i1p,i2p)

   !We sample dOmega_p1 (4pi)
   !We sample dOmega_pp1 (4pi) 
   !We sample dphi_p2 (2pi)
   !We sample pp1 (ppmax)
   r_now=r_now*2.0d0**2*(2.0d0*pi)**3 *ppmax!
   !write(6,*)'r_now = ', r_now
...
subroutine int_eval(kprobe_4,klept_4,p2,phip2,p1,ctp1, &
   ...
   real*8 :: w,p2,phip2,p1,ctp1,phip1,pp1,ctpp1,stpp1,phipp1,stp1,jac
   real*8 :: e1num(3),e1(3),e2(3),e3(3),K3(3),K3mag,K3hat(3) !Delta function stuff
   real*8 :: xhat(3),zhat(3),p2hat(3),A,cosgamma,singamma !Delta function stuff 
   real*8 :: q2,rho,norm,ca5,cv3,gep,np1
   real*8 :: p1_4(4),p2_4(4),pp1_4(4),pp2_4(4)
   real*8 :: kprobe_4(4),klept_4(4),q_4(4)
   ...
   !Here we utilize the momentum conserving delta function to
   !solve for \vec{pp2}, and the energy conserving delta function to
   !solve for costheta(p1)

   !Get sin theta for initial state nucleon 1
   stp1=sqrt(1.0d0-ctp1**2)

   !Ok I have defined p1
   ...
   !Solve the energy conserving delta function by fixing the
   !angle between K = (p1 + q + pp1) and p2

   !Define K3 = p1_3 + q_3 - pp1_3
   K3(:) = p1_4(2:4) +q_4(2:4) - pp1_4(2:4)

   !write(6,*)'K_3 = ', K3
   
   !Now define A which is equal to Epp2 (w + Ep1 + Ep2 - Epp1)
   A = q_4(1) + p1_4(1) + sqrt(p2**2+xmn**2) - pp1_4(1)

   !write(6,*)'A = Epp2 = ', A

   !Energy of final state nucleon 2 should be > Mn
   if(A.lt.xmn) then 
      r_now=czero 
      return
   endif

   K3mag = sqrt(dot_product(K3,K3))
   cosgamma = (A**2 - xmn**2 - K3mag**2 - p2**2)/2.0d0/K3mag/p2

   !write(6,*)'Cos(gamma) = ', cosgamma

   !cos Angle between between K3 and p2 should be between -1 and 1
   if(abs(cosgamma).gt.1.0d0) then 
      r_now=czero 
      return
   endif

   singamma = sqrt(1.0d0-cosgamma**2)
   K3hat = K3/K3mag

   !Set up coordinate system
   zhat = (/0.0d0,0.0d0,1.0d0/)
   xhat = (/1.0d0,0.0d0,0.0d0/)
   e3(:)=K3hat
   !Numerical check to make sure we don't get a bad cross product
   if(dot_product(zhat,K3hat).lt.0.999d0) then 
      e1num(:)=(zhat - dot_product(zhat,K3hat)*K3hat)
      e1(:)=e1num(:)/sqrt(dot_product(e1num,e1num))
   else 
      e1num(:)=(xhat - dot_product(xhat,K3hat)*K3hat)
      e1(:)=e1num(:)/sqrt(dot_product(e1num,e1num))
   endif

   e2 = cross_product(K3hat,e1)

   !Set up incoming nucleon 2 3_vector
   p2hat(:) = singamma*cos(phip2)*e1(:)+singamma*sin(phip2)*e2(:)+cosgamma*e3(:)
...
!....Pauli blocking
   ...
   !write(6,*)'do we ever make it here?'
