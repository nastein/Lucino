...
subroutine mc_init(gen_events_in,xsec_acc_in,i_fg_in,irn_int_in, &
   ...
   real*8 :: xpf_in,mlept_in,hp,he,dummy,xsec_acc_in
...
subroutine mc_eval(Enu, thetalept_in, thetaprot_in, w_in, xsec_tot, xsec_err_tot, my_events)
   use event_module
   use mathtool
   use dirac_matrices
   use mympi
   ...
   real*8 :: Enu,sig,thetalept_in,thetaprot_in,w_in
   ...
   real*8 :: q2_o(nwlk),q2_n(nwlk),w_n(nwlk),q2max_c
   ...
   do i=1,nwlk
      ...
      do while(g(i).le.0.0d0)
         ...
         do while (charged_conserved(i).eqv..FALSE.)
            ...
            i1(i)=int(2*ran() + 1)
            i2(i)=int(2*ran() + 1)
            !Setting outgoing particle 1 to always be a proton
            i1p(i)=1
            i2p(i)=int(2*ran() + 1)
...
subroutine f_eval(i1,i2,i1p,i2p,pj1,pj2,np1,elepi,f,my_event_in)
   use event_module
   use mathtool
   use mympi
   use dirac_matrices
   ...
   real*8 :: elepf,plepf,cos_theta,sin_theta
   real*8 :: phipp1,ctpp1,p2,ctp2,phip2,pj1,pj2,ctp1,phip1
   real*8 :: np1,elepi,f,jac_c,qval,q2
   ...
   elepf = elepi-w
   plepf = sqrt(elepf**2-mlept**2)
   cos_theta = cos(thetalept)
   sin_theta = sqrt(1.0d0 - cos_theta**2)
   q2 = 2.0d0*elepi*(elepf - plepf*cos_theta) - mlept**2
   ...
   probeP4(1) = elepi
   probeP4(2) = elepi*plepf*sin_theta/qval
   probeP4(3) = 0.0d0
   probeP4(4) = sqrt(elepi**2 - (elepi*plepf*sin_theta/qval)**2)

   outlepP4(1) = elepf
   outlepP4(2) = elepi*plepf*sin_theta/qval
   outlepP4(3) = 0.0d0
   ...
   sig0=10.0d0*hbarc**2 * alpha**2/(q2**2) * plepf/elepi * nu0

   !write(6,*)'omega = ', w 
   !write(6,*)'probe = ', probeP4
   !write(6,*)'lepton = ', outlepP4
   !write(6,*)'sig0 = ', sig0
   !write(6,*)'vl = ', vl, ', vtt = ', vtt  
   !write(6,*)'Rl = ', Rl, ', Rtt = ', Rtt

   !If doing contraction
   !.....compute sigma_mott [ fm^2 --> mb ]
   !sig0=10.0d0* hbarc**2 * alpha**2 * (elepf*plepf) /q2**2 

   sig = sig0*real(Vt*Rt + Vtt*Rtt)*1.e9
   ...
   my_event_in%responses(:) = (/Rcc,Rcl,Rll,Rt/)

   return
end subroutine f_eval

subroutine int_eval(kprobe_4,klept_4,phipp1,ctpp1,p2,ctp2,phip2,p1,ctp1, &
      &  phip1,q_4,r_now,np1,nuc1P4,nuc2P4,nuc1PP4,nuc2PP4, &
      &  i1,i2,i1p,i2p)
   use dirac_matrices         
   use mathtool
   implicit none
   ...
   real*8 :: phipp1,ctpp1,p2,ctp2,phip2,p1,ctp1,phip1,stpp1,stp1,stp2
...
subroutine g_eval(pj1,pj2,gPkE,gnorm,g)
   ...
   g=g/gnorm/3.0d0 !Dividing by 6 for the 6 different isospin combinations that satisfy charge conservation
