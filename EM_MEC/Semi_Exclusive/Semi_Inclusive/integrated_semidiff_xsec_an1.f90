module mc_module
   ...
   real*8, private, save:: wmax,qval,phiprot,w
...
subroutine mc_eval(Enu, qval_in, phiprot_in, w_in, xsec_tot, xsec_err_tot, my_events)
   use event_module
   use mathtool
   use dirac_matrices
   use mympi
   ...
   !Safety factor
   maximum_weight = global_max_weight*5.0d0
...
subroutine mc_calculate_xsec(Enu,i1,i2,i1p,i2p,j1,j2,g,i_avg,events,max_weight,r_avg,r_err,eventgen)
   ...
   if(mod(i1+i2,2).eq.0) then 
      call f_eval(i1,i2,i1p,i2p,p(j1),p(j2),dp1(j1,j2),&
      &  Enu,f,event)
   else
      call f_eval(i1,i2,i1p,i2p,p(j1),p(j2),dp0(j1,j2),&
      &  Enu,f,event)
   endif
...
subroutine f_eval(i1,i2,i1p,i2p,pj1,pj2,np1,enu_v,f,my_event_in)
   use event_module
   use mathtool
   use mympi
   use dirac_matrices
   ...
   call int_eval(probeP4,outlepP4,phiprot,ctpp1,pj2,ctp2,phip2, &
      &  pj1,ctp1,phip1,q,r_now,np1,nuc1P4,nuc2P4,nuc1PP4,nuc2PP4, &
      &  i1,i2,i1p,i2p)

   !I integrate over 2phis and 3 cosines
   r_now=r_now*3.0d0**2*(2.0d0*pi)**2
