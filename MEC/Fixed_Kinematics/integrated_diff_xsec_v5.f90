...
subroutine mc_calculate_xsec(Enu,i1,i2,i1p,i2p,j1,j2,w,g,i_avg,events,max_weight,r_avg,r_err,eventgen)
   ...
   if(mod(i1+i2,2).eq.0) then 
      call f_eval(w,i1,i2,i1p,i2p,p1(j1),p2(j2),dp1(j1,j2),&
      &  Enu,f,event,eventgen)
   else
      call f_eval(w,i1,i2,i1p,i2p,p1(j1),p2(j2),dp0(j1,j2),&
      &  Enu,f,event,eventgen)
   endif
...
subroutine f_eval(w,i1,i2,i1p,i2p,pj1,pj2,np1,enu_v,f,my_event_in,eventgen)
   use event_module
   use mathtool
   use mympi
   use dirac_matrices
   ...
   real*8 :: pj2,ctp2,stp2,phip2,pj1,ctp1,stp1,phip1,qrel_3(3),qtot_3(3)
...
subroutine int_eval(kprobe_4,klept_4,nuc1P4,nuc2P4, &
   ...
   real*8, parameter :: lsq=705600.0d0,l3=3.5d0*1.e6,xma2=1.1025d0*1.e6,xmad=950.0d0
   real*8, parameter :: fstar=2.14d0,eps=10.0d0,e_gs=-92.16,e_bg=-64.75
   real*8 :: w,pj1,pj2
